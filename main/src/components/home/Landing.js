...
const Landing = ({}) => {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="w-screen select-none bg-gradient-to-b from-orange-300 to-sky-50 relative h-screen overflow-hidden flex justify-center items-center"
    >
      {/* <motion.img
      ...
      <motion.div
        ...
        transition={{ delay: 1 }}
        className="bg-white max-w-[50%] flex flex-col  xs:max-w-max xs:w-5/6 overflow-hidden  w-[600px] rounded-xl p-4 bg-opacity-50"
      >
        <motion.img
        ...
        <p className="text-pretty flex w-full flex-col font-poppins font-bold text-white text-3xl xs:text-xl">
          <span className="text-orange-400">Empowering Your Rights,</span>
          <ReactTyped
            className="text-lime-600 text-4xl xs:text-2xl"
            strings={[
        ...
        <p className="text-xl xs:text-base my-4 font-lora font-bold text-slate-500">
          Welcome to{" "}
          <span className=" text-orange-600 text-3xl xs:text-xl ">efir</span>, the
