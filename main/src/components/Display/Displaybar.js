...
function Displaybar({ complaints, setComplaintList, setFilters, heading, myComplaints }) {
    ...
    <div className="w-full flex-grow transition-all duration-500">
      {!currentComplaint && (
        <div className="h-[90vh] flex-grow my-2 mr-2 transition-all duration-500">
          ...
          <div className="flex text-center justify-between font-bold" style={{ backgroundColor: "#AEDEFC" }}>
            <div className="w-[33%] md:w-[25%] p-2">Complaint Id's</div>
            <div className="w-[33%] md:w-[25%] p-2">Date of Filing</div>
            <div className="w-[33%] md:w-[25%] p-2 hidden md:block">District</div>
            ...
            {complaints &&
              ...
              complaints.complaints.map((complaint, index) => {
                ...
                return (
                  ...
                  <div
                    key={index}
                    className="flex text-center complaint-row"
                    style={{ position: "relative", backgroundColor: color }}
                    onMouseEnter={() => setHoveredIndex(index)}
                    onMouseLeave={() => setHoveredIndex(null)}
                  >
                    <div className="w-[33%] md:w-[25%] p-2">{complaint.firId}</div>
                    <div className="w-[33%] md:w-[25%] p-2">{complaint.LastEdited.slice(0, 10)}</div>
                    <div className="hidden md:block w-[25%] p-2">{complaint && complaint.IncidentDetail && complaint.IncidentDetail.District}</div>
                    ...
                    {myComplaints && hoveredIndex === index && (
                      <>
                      <div className="border-b-2 border-x-2 border-b-slate-200 shadow-sm rounded-b-xl mb-5 min-h-8    ">
                        <div className="font-semibold w-[20%] text-center">Status: {complaint.complaintStatus.status}</div>
